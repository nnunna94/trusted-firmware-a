...
/*
 * SMEM format version numbers (go/platforminfo). Each version adds fields at
 * the end of struct platforminfo_smem; only access fields whose version is <=
 * smem->format. Named by version number for direct cross-reference with the
 * go/platforminfo doc.
 */
...
/*
 * Per-instance Qultivate entry. An array of these is located at
 * anPartInstancesOffset within the SMEM buffer (format >= 23).
 * Each entry describes the fuse state of one instance of a hardware part.
 */
