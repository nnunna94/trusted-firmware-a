...
/*
 * Hardware part identifiers. Used to query whether a specific subsystem
 * is fused off on this SKU. Values are fixed by the SMEM disabled-features
 * table layout and must not be reordered.
 */
enum chipinfo_part {
...
/*
 * Returns true if the specified part instance is disabled on this SKU.
 * part_idx=0 checks the flat disabled-features table (all-or-nothing fusing).
 * part_idx>0 searches the per-instance Qultivate table (SMEM format >= 23).
 * Returns false (assume present) when uninitialized, out of range, or when
 * no matching entry exists in the per-instance table.
 */
